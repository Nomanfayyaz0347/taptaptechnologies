...
export default function ApproachSection() {
  return (
    <section id="services " style={{ backgroundColor: '#000000ff' }}>
      <div className="mi-invert-fix">
        <div className="container mil-p-120-60">
          <div className="row">
            <div className="col-lg-5">
              {/*
                DEBUG: If you see more than one SVG, check if <ApproachSection /> is being rendered multiple times in the parent or page file.
                This SVG should only appear once per ApproachSection instance.
              */}
              <div className="mil-lines-place mil-light">
                <svg id="unique-approach-svg" width="250" viewBox="0 0 300 1404" fill="none" xmlns="http://www.w3.org/2000/svg" className="mil-lines">
...
function ServiceCards() {
  ...
  useEffect(() => {
    fetch('https://taptaptechnologies.com/wp-json/wp/v2/services')
      .then((res) => {
  ...
  if (loading) return <div className="mil-light">Loading services...</div>;
  if (error) return <div className="mil-light">Error: {error}</div>;
  if (!services.length) return <div className="mil-light">No services found.</div>;
      ...
      {services.map((service, idx) => (
        <div className="col-md-6 col-lg-6" key={service.id}>
          <Link href={`/services/${service.slug}`} legacyBehavior>
            <a className={`mil-service-card-lg mil-more mil-accent-cursor${idx % 2 === 0 ? ' mil-offset' : ''}`}>
              <h4 className="mil-muted mil-up mil-mb-30" dangerouslySetInnerHTML={{ __html: service.title && service.title.rendered ? service.title.rendered.replace(/ and /g, ' and <br/>') : '' }} />
              {service.excerpt && service.excerpt.rendered && (
                <p className="mil-descr mil-light-soft mil-up mil-mb-10" dangerouslySetInnerHTML={{ __html: service.excerpt.rendered }} />
              )}
              {service.content && service.content.rendered && (
